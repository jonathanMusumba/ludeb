        ...
        <main class="main-content" id="mainContent">
            ...
            <div class="topbar">
                ...
                <div class="topbar-right">
                    ...
                    <a href="<?php echo $root_url; ?>logout.php?csrf_token=<?php echo $csrf_token; ?>" class="btn-logout">
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#resultsStatusTable tr[data-school-id]').each(function() {
                        const row = $(this);
                        const rowSchoolId = row.data('school-id');
                        const rowStatus = row.data('status');
                        const showBySchool = !schoolId || rowSchoolId == schoolId;
                        const showByStatus = !status || rowStatus === status;
                        row.css('display', showBySchool && showByStatus ? '' : 'none');
                    });
                }

                $('#resultsSchoolFilter, #resultsStatusFilter').change(applyResultsStatusFilter);
                $('#missingSchoolFilter').change(function() { applyFilter('missingTable', 'missingSchoolFilter'); });
                $('#entrantSchoolFilter').change(function() { applyFilter('entrantTable', 'entrantSchoolFilter'); });
                $('#divisionSchoolFilter').change(function() { applyFilter('summaryTable', 'divisionSchoolFilter'); });
                $('#subjectSchoolFilter').change(function() { applyFilter('subjectTrendChart', 'subjectSchoolFilter'); });

                // Export to CSV
                function exportToCsv(tableId, filename) {
                    const rows = $(`#${tableId} tr:visible`);
                    let csvContent = '';
                    rows.each(function() {
                ...
                $('#exportMissingCsv').click(function() { exportToCsv('missingTable', 'missing_marks'); });
                $('#exportEntrantCsv').click(function() { exportToCsv('entrantTable', 'entrant_monitoring'); });
                $('#exportSummaryCsv').click(function() { exportToCsv('summaryTable', 'division_summary'); });
                $('#exportResultsStatusCsv').click(function() { exportToCsv('resultsStatusTable', 'results_status'); });

                // Hide loading spinners
                $('.loading-spinner').hide();

                // Animation for content load
                $('.dashboard-card, .page-title, .filter-container').css('animation', 'fadeIn 0.5s ease');
            },
            error: function(xhr, status, error) {
