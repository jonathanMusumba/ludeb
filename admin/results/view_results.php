...
$result = $conn->query("SELECT id, school_name, center_no FROM schools ORDER BY school_name");
$schools = [];
...
if ($search_term) {
    $stmt = $conn->prepare("SELECT id, school_name, center_no FROM schools 
                           WHERE school_name LIKE ? OR center_no LIKE ?");
...
if ($school_id) {
    $stmt = $conn->prepare("
        SELECT c.index_number, c.candidate_name,
        ...
        WHERE r.school_id = ? AND r.exam_year_id = ?
    ...
    $stmt = $conn->prepare("
        ...
        WHERE school_id = ? AND exam_year_id = ?
        GROUP BY division
    ");
    $stmt->bind_param('ii', $school_id, $exam_year_id);
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $summary[] = $row;
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    $school_name = $result->fetch_assoc()['school_name'] ?? '';
    $stmt->close();
}

// Export to Excel
if (isset($_GET['export']) && $_GET['export'] === 'excel' && $school_id) {
    $spreadsheet = new Spreadsheet();
    $sheet = $spreadsheet->getActiveSheet();
    $sheet->setTitle('School Results');
    ...
    header('Content-Disposition: attachment;filename="school_results_' . $school_id . '_' . $exam_year_id . '.xlsx"');
    $writer->save('php://output');
    exit();
}

// Export to PDF
if (isset($_GET['export']) && $_GET['export'] === 'pdf' && $school_id) {
    $dompdf = new Dompdf();
    $html = '
    <style>
    ...
    $dompdf->render();
    $dompdf->stream('school_results_' . $school_id . '_' . $exam_year_id . '.pdf', ['Attachment' => true]);
    exit();
}

// Capture content for layout.php
$content = '
<div class="page-header">
    <h1 class="page-title">School Results</h1>
...
<form method="GET" class="filter-container">
    <div class="row g-3 align-items-center">
        ...
        <div class="col-auto">
            <input type="text" name="search" value="' . htmlspecialchars($search_term) . '" class="form-control" placeholder="School name or center number">
        </div>
        <div class="col-auto">
            <select name="school_id" onchange="this.form.submit()" class="form-select">
                <option value="">--Select School--</option>';
foreach ($schools as $school) {
    $content .= '<option value="' . $school['id'] . '" ' . ($school_id == $school['id'] ? 'selected' : '') . '>' 
                . htmlspecialchars($school['school_name'] . ' (' . $school['center_no'] . ')') . '</option>';
}
$content .= '
            </select>
        </div>
        ...
        <div class="col-auto">
            <select name="exam_year_id" onchange="this.form.submit()" class="form-select">';
...
while ($year = $result->fetch_assoc()) {
    $content .= '<option value="' . $year['id'] . '" ' . ($exam_year_id == $year['id'] ? 'selected' : '') . '>' 
                . $year['exam_year'] . ($year['id'] == $active_year['id'] ? ' (Active)' : '') . '</option>';
}
$content .= '
            </select>
        </div>
        <div class="col-auto">
            <button type="submit" class="btn-enhanced btn-sm"><i class="fas fa-search"></i> Search</button>
        </div>
    </div>
</form>';
...
if ($school_id) {
    ...
    <div class="mb-3">
        <a href="?school_id=' . $school_id . '&exam_year_id=' . $exam_year_id . '&export=pdf" class="btn-enhanced btn-sm"><i class="fas fa-file-pdf"></i> Download PDF</a>
        <a href="?school_id=' . $school_id . '&exam_year_id=' . $exam_year_id . '&export=excel" class="btn-enhanced btn-sm"><i class="fas fa-file-excel"></i> Download Excel</a>
    </div>
