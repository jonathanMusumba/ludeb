...
jQuery(document).ready(function($) {
    ...
    window.showNotification = window.showNotification || function(message, type) {
        ...
        var notification = $('<div>').addClass('alert alert-enhanced ' + alertClass)
                                    .html(icon + ' ' + message)
                                    .css({
                                        position: 'fixed',
                                        top: '20px',
    ...
    $('#uploadForm').on('submit', function(e) {
        ...
        $.ajax({
            url: 'schools/upload_candidates.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    var newRow = `
                        <tr data-upload-id="${response.upload_id}">
                            <td>${response.filename}</td>
                            <td>${response.uploaded_at}</td>
                            <td>
                                <a href="schools/download_file.php?id=${response.upload_id}" class="btn btn-enhanced btn-sm me-1" title="Download">
                                    <i class="fas fa-download"></i>
                                </a>
                                <button class="btn btn-enhanced btn-sm delete-btn" 
                                        data-upload-id="${response.upload_id}" 
                                        data-csrf="<?php echo htmlspecialchars($csrf_token); ?>" 
                                        title="Delete">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                    ...
                    `;
                    var $tableBody = $('#uploadsTable tbody');
                    if ($tableBody.find('tr td[colspan="3"]').length > 0) {
                    ...
                    // Reset form
                    $('#uploadForm')[0].reset();
                    $('#exam_year_id').val('<?php echo $default_exam_year_id; ?>');
                } else {
                    window.showNotification(response.message || 'Failed to upload candidates.', 'error');
                }
            },
            error: function(xhr, status, error) {
                console.log('Upload AJAX Error:', xhr.responseText);
                window.showNotification('Failed to upload candidates: ' + error, 'error');
            }
        });
    });
