...
-- One-time script to fix existing data in candidate_results
START TRANSACTION;

-- Step 1: Standardize 'U' to 'Ungraded' in candidate_results
UPDATE candidate_results
SET division = 'Ungraded'
WHERE division = 'U';

-- Step 2: Correct divisions for candidates with aggregates 29
UPDATE candidate_results cr
JOIN (
    SELECT 
        r.candidate_id,
        r.exam_year_id,
        SUM(r.score) AS aggregates,
        SUM(CASE WHEN r.score <= 8 THEN 1 ELSE 0 END) AS pass_count,
        MAX(CASE WHEN s.code = 'ENG' THEN r.score ELSE 9 END) AS eng_score,
        MAX(CASE WHEN s.code = 'MTC' THEN r.score ELSE 9 END) AS mtc_score
    FROM results r
    JOIN subjects s ON r.subject_id = s.id
    JOIN candidates c ON r.candidate_id = c.id
    WHERE r.exam_year_id = c.exam_year_id
    GROUP BY r.candidate_id, r.exam_year_id
    HAVING COUNT(CASE WHEN r.score <= 9 THEN 1 END) >= 3
) calc ON cr.candidate_id = calc.candidate_id AND cr.exam_year_id = calc.exam_year_id
SET cr.division = (
    SELECT g.division
    FROM grades g
    WHERE calc.aggregates BETWEEN g.aggregate_range_from AND g.aggregate_range_to
    AND g.exam_year_id = cr.exam_year_id
    AND (
        g.conditions IS NULL
        OR g.conditions = '{}'
        OR (
            JSON_EXTRACT(g.conditions, '$.min_pass_subjects') = 3
            AND calc.pass_count >= 3
            AND (
                JSON_CONTAINS(JSON_EXTRACT(g.conditions, '$.must_include'), '"ENG"', '$')
                AND calc.eng_score <= 8
                OR JSON_CONTAINS(JSON_EXTRACT(g.conditions, '$.must_include'), '"MTC"', '$')
                AND calc.mtc_score <= 8
            )
        )
        OR (
            JSON_EXTRACT(g.conditions, '$.min_pass_subjects') = 2
            AND calc.pass_count >= 2
            AND calc.aggregates = 29
        )
    )
    ORDER BY g.aggregate_range_from ASC
    LIMIT 1
)
WHERE cr.aggregates = 29
AND cr.division = 'Ungraded';

-- Step 3: Correct divisions for candidates with aggregates 33-36
UPDATE candidate_results cr
JOIN (
    SELECT 
        r.candidate_id,
...
SET cr.division = (
    ...
    AND (
        ...
        OR (
            JSON_EXTRACT(g.conditions, '$.max_pass_subjects') = 1
            AND calc.pass_count <= 1
            AND calc.aggregates BETWEEN 33 AND 36
        )
    )
    ORDER BY g.aggregate_range_from ASC
    LIMIT 1
)
WHERE cr.aggregates BETWEEN 33 AND 36
AND cr.division NOT IN ('Division 4', 'Ungraded');

-- Step 4: Set division to 'X' for candidates with fewer than 3 PRESENT subjects
UPDATE candidate_results cr
JOIN (
    SELECT 
        m.candidate_id,
        m.exam_year_id,
        COUNT(CASE WHEN m.status = 'PRESENT' THEN 1 END) AS subject_count
    FROM marks m
    GROUP BY m.candidate_id, m.exam_year_id
) calc ON cr.candidate_id = calc.candidate_id AND cr.exam_year_id = calc.exam_year_id
SET cr.division = 'X', cr.aggregates = 0
WHERE calc.subject_count < 3
AND cr.division != 'X';

-- Step 5: Log the fix
INSERT INTO audit_logs (action, user_id, details, created_at)
SELECT 'FixCandidateResults', NULL, 
       CONCAT('Fixed divisions for candidate_id: ', cr.candidate_id, 
              ', exam_year_id: ', cr.exam_year_id, 
              ', from division: ', cr.division, 
              ', to division: ', (
                  SELECT g.division
                  FROM grades g
                  WHERE calc.aggregates BETWEEN g.aggregate_range_from AND g.aggregate_range_to
...
WHERE cr.aggregates = 29 OR cr.division = 'U' OR cr.aggregates BETWEEN 33 AND 36;

-- Step 6: Recompute school results status for all affected schools
CALL ProcessAllCandidates((SELECT id FROM exam_years WHERE status = 'Active' LIMIT 1), NULL);
