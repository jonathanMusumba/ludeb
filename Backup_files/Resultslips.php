...
function generateResultSlips($school_id) {
    ...
    $pdf->Image('logo.png', 150, 10, 30); // Add watermark (ensure logo.png is in the directory)

    while ($candidate = $candidates->fetch_assoc()) {
        if ($count % 3 == 0 && $count > 0) {
            $pdf->AddPage();
            $y_start = 10;
            $pdf->Image('logo.png', 150, 10, 30); // Add watermark to new page
        }
        ...
        if ($aggregates['has_absence']) {
            $division = 'X';
            $aggregates['total_aggregate'] = 'X';
        }
        ...
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(45, $line_height, 'School:', 0);
        $pdf->Cell(0, $line_height, $candidate['school_name'], 0, 1);
        $pdf->Cell(45, $line_height, 'Candidate:', 0);
        $pdf->Cell(0, $line_height, $candidate['Candidate_Name'], 0, 1);
        $pdf->Cell(45, $line_height, 'Sex:', 0);
        $pdf->Cell(0, $line_height, $candidate['Gender'], 0, 1);
        $pdf->Cell(45, $line_height, 'Index No:', 0);
        $pdf->Cell(0, $line_height, $candidate['IndexNo'], 0, 1);
        ...
        $pdf->Cell(30, $line_height, 'Official Stamp', 1);
        $pdf->Ln();
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $pdf->Cell(30, $line_height, '', 1); // Placeholder for official stamp
            $pdf->Ln();
        }
