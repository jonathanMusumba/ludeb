            ...
            foreach ($sheet->getRowIterator() as $row) {
                ...
                if (count($data) === 3) {
                    ...
                    if (!in_array($gender, ['M', 'F'])) {
                        echo "Invalid gender value: $gender for IndexNo $index_no in file $filename<br>";
                        continue;
                    }
                    ...
                    if ($school_result->num_rows === 0) {
                        echo "No school found for CenterNo $center_no in file $filename.<br>";
                        continue;
                    }
                    ...
                    if (!$stmt->execute()) {
                        echo "Error inserting row in file $filename: " . $stmt->error . "<br>";
                    }
                } else {
                    echo "Row data does not match expected format in file $filename: " . implode(", ", $data) . "<br>";
                }
            }
            echo "Candidates from $filename uploaded successfully.<br>";

        } catch (Exception $e) {
            echo "Failed to process $filename: " . $e->getMessage() . "<br>";
        }
    }
} else {
