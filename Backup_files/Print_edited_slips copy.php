...
function generateResultSlipsForUpdatedRecords() {
    ...
    while ($school = $schools_result->fetch_assoc()) {
        ...
        while ($candidate = $candidates->fetch_assoc()) {
            ...
            $serial_number = $current_year . $candidate_id . $index_no_last_two . '2'; // Added '2' for second copy

            // Draw slip content
            $pdf->SetXY($x_start, $y_start);
            $pdf->SetFont('Arial', 'B', 10); // Set bold font for headers
            ...
            $pdf->SetFont('Arial', 'B', 9); // Set bold font for other headers
            ...
            $pdf->Ln(5);
            // Table Header
            $pdf->SetFont('Arial', 'B', 9); // Set bold font for table headers
            ...
            $marks_query = "SELECT s.Name, m.mark, g.score 
                            ...
                            WHERE m.candidate_id = ?
                            ORDER BY FIELD(s.id, 1, 3, 2, 4)"; // Ensure consistent subject order
            $stmt = $conn->prepare($marks_query);
            $stmt->bind_param("i", $candidate_id);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            $pdf->Cell(30, $line_height, number_format($average_mark, 1), 0, 1);
