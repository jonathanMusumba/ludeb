...
$exam_year_id = $row['exam_year_id'] ?? 0; // Store active exam_year_id
        ...
        <div class="col-md-6">
            <form method="GET" action="" id="schoolSearchForm">
                <div class="table-filter">
                    ...
                    <input type="text" id="school_search" name="school_search" class="form-control" placeholder="Search by center number or school name" value="<?php echo isset($_GET['school_search']) ? htmlspecialchars($_GET['school_search']) : ''; ?>" required>
...
$(document).ready(function() {
    ...
    $("#school_search").autocomplete({
        source: function(request, response) {
            $.ajax({
                ...
                success: function(data) {
                    console.log("Autocomplete response:", data); // Debug response
                    response(data);
                },
                error: function(xhr, status, error) {
                    console.error("Autocomplete error:", status, error, xhr.responseText);
                    window.showNotification("Failed to load school suggestions", "error");
                    response([]);
                }
            });
        },
        ...
        select: function(event, ui) {
            $("#school_search").val(ui.item.value); // Display the selected value
            $("#school_id").val(ui.item.id); // Set the hidden school_id
            $("#schoolSearchForm").submit(); // Submit the form
            return false;
        },
        open: function() {
            console.log("Autocomplete menu opened"); // Debug menu open
        }
    ...
    $(".mark-input").on("keydown", function(e) {
        if (e.key === "Enter") {
            ...
            } else if (mark !== "" || checkbox.is(":checked")) {
                saveMark(candidateId, input, checkbox);
            }

            // Focus next non-disabled, non-locked input
