            ...
            <div>
                ...
                <ul class="space-y-1">
                    ...
                    foreach ($classes as $value => $label): 
                        $class_count_sql = "SELECT COUNT(*) as count FROM resources WHERE approved = 1 AND YEAR(created_at) = ?" . ($value !== 'all' ? " AND class = ?" : "");
                        $class_count_stmt = $conn->prepare($class_count_sql);
                        if ($value === 'all') {
                            $class_count_stmt->bind_param("i", $current_year);
                        } else {
                            $class_count_stmt->bind_param("is", $current_year, $value);
                        }
                        $class_count_stmt->execute();
                        $class_count = $class_count_stmt->get_result()->fetch_assoc()['count'];
                        $class_count_stmt->close();
                    ?>
                    <li>
                        <a href="?year=<?php echo $current_year; ?>&class=<?php echo $value; ?>&filter=<?php echo $filter; ?>&category=<?php echo $category_filter; ?>&search=<?php echo urlencode($search); ?>" 
                           class="block p-2 rounded hover:bg-blue-50 transition-colors <?php echo $class_filter === $value ? 'bg-blue-100 text-blue-600 font-medium' : 'text-gray-700'; ?>">
                            <?php echo $label; ?>
                            <span class="badge-circle"><?php echo $class_count; ?></span>
                        </a>
                    </li>
                    <?php endforeach; ?>
                </ul>
            ...
            <div>
                ...
                <ul class="space-y-1">
                    ...
                    foreach ($categories as $value => $label): 
                        $category_count_sql = "SELECT COUNT(*) as count FROM resources WHERE approved = 1 AND YEAR(created_at) = ?" . ($value !== 'all' ? " AND category = ?" : "");
                        $category_count_stmt = $conn->prepare($category_count_sql);
                        if ($value === 'all') {
                            $category_count_stmt->bind_param("i", $current_year);
                        } else {
                            $category_count_stmt->bind_param("is", $current_year, $value);
                        }
            ...
            <div>
                ...
                <ul class="space-y-1">
                    ...
                    foreach ($types as $value => $label): 
                        $type_count_sql = "SELECT COUNT(*) as count FROM resources WHERE approved = 1 AND YEAR(created_at) = ?" . ($value !== 'all' ? " AND type = ?" : "");
                        $type_count_stmt = $conn->prepare($type_count_sql);
                        if ($value === 'all') {
                            $type_count_stmt->bind_param("i", $current_year);
                        } else {
                            $type_count_stmt->bind_param("is", $current_year, $value);
                        }
                        $type_count_stmt->execute();
                        $type_count = $type_count_stmt->get_result()->fetch_assoc()['count'];
                    ...
                    <li>
                        <a href="?year=<?php echo $current_year; ?>&filter=<?php echo $value; ?>&class=<?php echo $class_filter; ?>&category=<?php echo $category_filter; ?>&search=<?php echo urlencode($search); ?>" 
                           class="block p-2 rounded hover:bg-blue-50 transition-colors <?php echo $filter === $value ? 'bg-blue-100 text-blue-600 font-medium' : 'text-gray-700'; ?>">
                            <?php echo $label; ?>
                            <span class="badge-circle"><?php echo $type_count; ?></span>
                        </a>
                    </li>
                    <?php endforeach; ?>
                </ul>
                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex flex-col gap-2 ml-4">
                        ...
                        <?php elseif ($resource['type'] === 'premium'): ?>
                            ...
                            <?php else: ?>
                                <a href="payment.php?resource_id=<?php echo $resource['id']; ?>" 
