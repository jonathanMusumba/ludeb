...
try {
    ...
    function getBrowser() {
        ...
        // Note: Order matters - check Safari before Chrome since Chrome UA contains "Safari"
    ...
    function logLoginAttempt($conn, $user_id, $status, $ip_address, $operating_system, $browser) {
        ...
        if ($stmt) {
            // Use 'i' for integer user_id, but handle NULL case
            if ($user_id === null) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $conn->prepare("SELECT id, username, email, password, role, status, school_id FROM system_users WHERE username = ? OR email = ?");
        if (!$stmt) {
        ...
        if ($result->num_rows == 1) {
            ...
            if ($row['status'] === 'Invalid') {
                // Use prepared statements for logging too
                $logStmt = $conn->prepare("CALL log_action(?, ?, ?)");
                if ($logStmt) {
                    $action = 'Login Attempt';
                    $user_id_log = null;
                    $message = 'Failed: Invalid account status for ' . $row['username'];
                    $logStmt->bind_param("sis", $action, $user_id_log, $message);
                    $logStmt->execute();
                    $logStmt->close();
                }
                
                logLoginAttempt($conn, $row['id'], 'Failed', $ip_address, $operating_system, $browser);
            ...
            if (password_verify($pass, $row['password'])) {
                ...
                if ($logStmt) {
                    ...
                    $message = 'User ' . $row['username'] . ' logged in';
                    $logStmt->bind_param("sis", $action, $row['id'], $message);
                    $logStmt->execute();
                    $logStmt->close();
                }
                
                logLoginAttempt($conn, $row['id'], 'Success', $ip_address, $operating_system, $browser);
                ...
                if ($logStmt) {
                    $action = 'Login Attempt';
                    $user_id_log = null;
                    $message = 'Failed: Invalid password for ' . $row['username'];
                    $logStmt->bind_param("sis", $action, $user_id_log, $message);
                    $logStmt->execute();
                    $logStmt->close();
                }
            ...
            if ($logStmt) {
                $action = 'Login Attempt';
                $user_id_log = null;
                $message = 'Failed: No user found for ' . $user;
                $logStmt->bind_param("sis", $action, $user_id_log, $message);
                $logStmt->execute();
                $logStmt->close();
            }
    ...
    $error = $e->getMessage();
    error_log("Login error: " . $e->getMessage()); // Log to server error log
} finally {
    // Clean up resources
