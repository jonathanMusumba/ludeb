...
$tables_check = $conn->query("SHOW TABLES LIKE 'candidates'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table candidates does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table candidates is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
";
$schools = [];
$tables_check = $conn->query("SHOW TABLES LIKE 'schools'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table schools does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table schools is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
$tables_check = $conn->query("SHOW TABLES LIKE 'school_types'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table school_types does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table school_types is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
$worst_schools = array_slice($schools, 0, 10);

// Derive schools with first grades
$first_grade_schools = array_filter($schools, function($school) {
    return $school['division_1_count'] > 0;
});

// Get schools with first grades query
...
$best_candidates_query = "
    ...
    WHERE cr.exam_year_id = ? AND cr.aggregates >= 4 AND cr.aggregates <= 10
    ORDER BY cr.aggregates ASC, c.candidate_name ASC
    LIMIT 5
";
$best_candidates = [];
...
while ($row = $result->fetch_assoc()) {
    ...
    error_log("detailed_report.php: Fetched candidate: " . $row['candidate_name'] . " with aggregate: " . $row['aggregates'], 3, __DIR__ . '/logs/setup_errors.log');
}
$stmt->close();
...
";
$subject_performance = [];
$tables_check = $conn->query("SHOW TABLES LIKE 'marks'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table marks does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table marks is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
$tables_check = $conn->query("SHOW TABLES LIKE 'subjects'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table subjects does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table subjects is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
$tables_check = $conn->query("SHOW TABLES LIKE 'grading'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table grading does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table grading is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
$tables_check = $conn->query("SHOW TABLES LIKE 'subcounties'");
if ($tables_check->num_rows == 0) {
    error_log("detailed_report.php: Table subcounties does not exist", 3, __DIR__ . '/logs/setup_errors.log');
    $content = '<div class="alert alert-danger fade-in">Error: Database table subcounties is missing. Please contact the administrator.</div>';
    renderLayout('Error', [
        'title' => 'Error',
        'content' => $content,
        'current_page' => 'detailed_report',
...
if (isset($_GET['download']) && $_GET['download'] === 'pdf') {
    ...
    $pdf->Ln(5);
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 6, 'General Performance by Schools', 0, 1, 'L');
    $headers = ['SN', 'School Name', 'School Type', 'Div 1 (M|F)', 'Div 2 (M|F)', 'Div 3 (M|F)', 'Div 4 (M|F)', 'U (M|F)', 'X (M|F)', 'Total', '% Pass', '% Fail'];
    $widths = [10, 50, 20, 25, 25, 25, 25, 25, 25, 20, 20, 20];
    $pdf->SetFillColor(200, 220, 255);
    foreach ($headers as $i => $header) {
        $pdf->Cell($widths[$i], 6, $header, 1, 0, 'C', true);
    }
    ...
    // Candidates Performance: Best Candidates
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 8, 'Candidates Performance', 0, 1, 'C');
    $pdf->Ln(5);
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 6, 'Best Candidates (Top 5 by Aggregates)', 0, 1, 'L');
    $pdf->SetFont('Arial', 'I', 8);
    $pdf->Cell(0, 6, 'Note: Aggregates range from 4 (best) to 36 (worst). Lower aggregates indicate better performance.', 0, 1, 'L');
    $headers = ['SN', 'Index No', 'Candidate Name', 'Aggregates', 'School'];
    $widths = [10, 25, 50, 20, 50];
    $pdf->SetFillColor(200, 220, 255);
    $pdf->SetFont('Arial', 'B', 10);
    foreach ($headers as $i => $header) {
        $pdf->Cell($widths[$i], 6, $header, 1, 0, 'C', true);
    ...
    if (empty($best_candidates)) {
        ...
        foreach ($best_candidates as $i => $candidate) {
            $pdf->Cell($widths[0], 6, $i + 1, 1, 0, 'C');
            ...
            $pdf->Cell($widths[4], 6, substr($candidate['school_name'], 0, 25), 1, 0, 'L');
            $pdf->Ln();
        }
    }
    $pdf->Ln(10);
...
if (isset($_GET['download']) && $_GET['download'] === 'word') {
   ...
   $section->addText('Candidates Performance', ['bold' => true, 'size' => 12], ['alignment' => 'center']);
    $section->addTextBreak(1);
    $section->addText('Best Candidates (Top 5 by Aggregates)', ['bold' => true, 'size' => 10]);
    $section->addText('Note: Aggregates range from 4 (best) to 36 (worst). Lower aggregates indicate better performance.', ['italic' => true, 'size' => 8]);
    $table = $section->addTable(['borderSize' => 6, 'borderColor' => '006699', 'cellMargin' => 80]);
    $table->addRow();
    foreach (['SN', 'Index No', 'Candidate Name', 'Aggregates', 'School'] as $header) {
        $table->addCell(2000)->addText($header, ['bold' => true]);
    }
    if (empty($best_candidates)) {
        $table->addRow();
...
foreach ($subcounties_performance as $i => $subcounty) {
    $content .= '
                <tr>
                    ...
                    <td>' . $subcounty['pass_percentage'] . '</td>
                    <td>' . $subcounty['fail_percentage'] . '</td>
                    <td>' . $subcounty['rank'] . '</td>
                </tr>';
}
$content .= '
            </tbody>
    ...
    <h6 class="mt-4">Subcounty Pass Percentage</h6>
    <div style="height: 400px;">
        <canvas id="subcountyChart"></canvas>
    </div>
</div>

<!-- General Performance by Schools -->
<div class="chart-card fade-in">
    <h5>General Performance by Schools</h5>
    <div class="table-responsive">
        <table class="table table-striped table-bordered">
            <thead>
                <tr>
...
<div class="chart-card fade-in">
    <h5>School Performance</h5>
    <h6>Schools with First Grades</h6>
    <div class="table-responsive">
        <table class="table table-striped table-bordered">
            <thead>
                <tr>
