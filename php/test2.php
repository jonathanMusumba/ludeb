...
// Prepare SQL for fetching foreign key values
$subCountySql = "SELECT id FROM sub_counties WHERE subcounty = ?";
$subCountyStmt = $conn->prepare($subCountySql);
if (!$subCountyStmt) {
    die("Prepare failed: " . $conn->error);
}

$schoolTypeSql = "SELECT id FROM school_type WHERE type = ?";
$schoolTypeStmt = $conn->prepare($schoolTypeSql);
if (!$schoolTypeStmt) {
    die("Prepare failed: " . $conn->error);
}
...
foreach ($data as $row) {
    ...
    if ($checkStmt->num_rows === 0) {
        // Fetch sub_county ID
        $subCountyStmt->bind_param('s', $subCountyName);
        $subCountyStmt->execute();
        $subCountyStmt->bind_result($subCountyId);
        $subCountyStmt->fetch();
        $subCountyStmt->free_result();
        
        if (!$subCountyId) {
            echo "Sub_county not found: $subCountyName<br>";
            continue; // Skip to the next record
        }

        // Fetch school_type ID
        $schoolTypeStmt->bind_param('s', $schoolTypeName);
        $schoolTypeStmt->execute();
        $schoolTypeStmt->bind_result($schoolTypeId);
        $schoolTypeStmt->fetch();
        $schoolTypeStmt->free_result();
        
        if (!$schoolTypeId) {
            echo "School_type not found: $schoolTypeName<br>";
            continue; // Skip to the next record
        }

        // Insert new record if it doesn't exist
        $insertStmt->bind_param('ssis', $centerNo, $schoolName, $subCountyId, $schoolTypeId);
